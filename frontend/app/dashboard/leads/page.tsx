...
const PRIORITY_TABS = [
  { value: '', label: 'All Leads', icon: '👥' },
  { value: 'high', label: '🔥 Hot', icon: '' },
  { value: 'medium', label: '⚡ Medium', icon: '' },
  { value: 'low', label: '🟡 Low', icon: '' },
  { value: 'sales_pitch', label: '📢 Pitches', icon: '', isStatus: true },
];
      ...
      <div className="flex gap-2 mb-6 bg-white rounded-2xl p-1 border border-slate-100 shadow-sm w-fit">
        {PRIORITY_TABS.map(tab => (
          <button key={tab.value}
            ...
            className={`px-4 py-2 rounded-xl text-sm font-medium transition ${activeTab === tab.value ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}>
            {tab.label}
          </button>
        ))}
      </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {leads.map(lead => (
                  <tr key={lead.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-5 py-4">
                      {lead.priority && (
                        <span className={`text-xs px-2 py-1 rounded-full font-medium ${PRIORITY_BADGE[lead.priority] || 'bg-slate-100 text-slate-600'}`}>
                          {lead.priority === 'high' ? '🔥 Hot' : lead.priority === 'medium' ? '⚡ Med' : '🟡 Low'}
                        </span>
                      )}
                    </td>
                    <td className="px-5 py-4">
                      {lead.lead_score > 0 && (
                        <div className="flex items-center gap-1">
                          <div className="w-16 bg-slate-100 rounded-full h-1.5">
                            <div className={`h-1.5 rounded-full ${lead.lead_score >= 70 ? 'bg-red-500' : lead.lead_score >= 40 ? 'bg-yellow-500' : 'bg-slate-400'}`}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <Link href={`/dashboard/leads/${lead.id}`} className="px-3 py-1 text-xs bg-indigo-50 text-indigo-700 rounded-lg hover:bg-indigo-100 font-medium transition">View</Link>
                        <button onClick={() => deleteLead(lead.id)} className="px-2 py-1 text-xs text-red-500 hover:bg-red-50 rounded-lg transition">🗑</button>
                      </div>
                    </td>
                  </tr>
                ))}
